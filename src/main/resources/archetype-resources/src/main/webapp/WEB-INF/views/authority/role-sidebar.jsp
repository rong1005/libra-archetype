					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="box border primary">
								...
								<div class="box-body">
									<form id="inputForm" class="form-horizontal" action="${symbol_dollar}{ctx}/role/sidebar" method="post">
										<input type="hidden" name="id" value="${symbol_dollar}{role.id}"/>
										
										<c:forEach items="${symbol_dollar}{topSidebar }" var="top">
										<div class="form-group">
											<div class="col-sm-2" style="text-align: right;">
												<label class="checkbox-inline"> <input type="checkbox" name="sidebars.id" class="uniform" value="${symbol_dollar}{top.id}" <c:if test="${symbol_dollar}{isExist[top.id] }">checked="checked"</c:if>>${symbol_dollar}{top.name}</label>
											</div>
										</div>
										
										<c:if test="${symbol_dollar}{not empty top.sidebars }">
										<div class="form-group">
											<div class="col-sm-offset-2 col-sm-10">
												<c:forEach items="${symbol_dollar}{top.sidebars}" var="child">
												<label class="checkbox-inline"> <input type="checkbox" name="sidebars.id" class="uniform" value="${symbol_dollar}{child.id}" <c:if test="${symbol_dollar}{isExist[child.id] }">checked="checked"</c:if>>${symbol_dollar}{child.name}</label>
												</c:forEach>
											</div>
										</div>
										</c:if>
