...
<body>
								...
								<div class="box-body">
									...
									<div class="row">
										<div class="col-sm-12">
											...
											<div class="pull-left">
												<a class="btn btn-info" href="${symbol_dollar}{ctx}/department/create">创建部门</a>
											</div>
										</div>
									</div>
								</div>
	...
	<!-- bootstrap-tree -->
	<script src="${symbol_dollar}{ctx}/static/js/bootstrap-tree/bootstrap.tree.js"></script>
	<!-- bootstrap-table -->
	<script src="${symbol_dollar}{ctx}/static/js/bootstrap-table/bootstrap.table.js"></script>
	...
	<script>
		...
		jQuery(document).ready(function() {
			...
			jQuery.ajax({
				url: "${symbol_dollar}{ctx}/department/tree",
				type: "post",
				dataType: "json",
				success: function(msg) {
					...
					Tree.onClick(function(node){
						${symbol_dollar}("#nodeid").val(node.id);
						Table.load("${symbol_dollar}{ctx}/department?sortType="+${symbol_dollar}("#sortTypeKey").val()+searchParams());
					});
				}
			});
			...
			Table.setHead(thead);
			Table.load("${symbol_dollar}{ctx}/department");

		});
		
		//分页
		function page(page){
			Table.load("${symbol_dollar}{ctx}/department?sortType="+${symbol_dollar}("#sortTypeKey").val()+"&page="+page+searchParams());
		};
		...
		function sort(sortType,value){
			//调用时，产生loding的效果.
			App.blockUI(${symbol_dollar}(".table-page").parents(".box"));
			
			${symbol_dollar}("#sortTypeValue").html(value);
			${symbol_dollar}("#sortTypeKey").val(sortType);
			Table.load("${symbol_dollar}{ctx}/department?sortType="+sortType+searchParams());
			
			window.setTimeout(function () {
                App.unblockUI(${symbol_dollar}(".table-page").parents(".box"));
            }, 1000);
		}
		...
		function searchParams(){
			var searchParams='';
			if(${symbol_dollar}("#nodeid").val()!=""){
				searchParams=searchParams+'&nodeid='+${symbol_dollar}("#nodeid").val();
			}
			if(${symbol_dollar}("#name").val()!=""){
				searchParams=searchParams+'&name='+${symbol_dollar}("#name").val();
			}
			return searchParams;
		}
