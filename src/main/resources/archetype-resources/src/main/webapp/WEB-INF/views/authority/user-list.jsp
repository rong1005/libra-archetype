								...
								<div class="box-body">
									<form class="form-inline" action="#">
										<div class="form-group">
											<input type="text" name="search_LIKE_name" class="form-control" value="${symbol_dollar}{param.search_LIKE_name}" placeholder="姓名" />
										</div>
									...
									<table  class="table table-striped table-bordered table-hover">
										...
										<c:forEach items="${symbol_dollar}{users.content}" var="user">
											<tr>
												<td>${symbol_dollar}{user.loginName}</td>
												<td>${symbol_dollar}{user.name}</td>
												<td>${symbol_dollar}{user.email}</td>
												<td><fmt:formatDate value="${symbol_dollar}{user.registerDate}" pattern="yyyy年MM月dd日" /></td>
												<td><a href="${symbol_dollar}{ctx}/user/update/${symbol_dollar}{user.id}">修改</a> / <a href="${symbol_dollar}{ctx}/user/delete/${symbol_dollar}{user.id}" onclick="return confirm('是否删除该用户？')">删除</a></td>
											</tr>
										</c:forEach>
										</tbody>
									</table>
									<div class="row">
										<div class="col-sm-12">
											...
											<div class="pull-left">
												<a class="btn btn-info in-loading" href="${symbol_dollar}{ctx}/user/create">创建用户</a>
											</div>
										</div>
									</div>
								</div>
