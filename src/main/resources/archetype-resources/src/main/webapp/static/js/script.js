var App = function () {
	...
	var handleSidebarCollapse = function () {
		...
		//如果侧边栏的样式为空，设置一个默认的样式；（cookie在登录页删除了）
		if ($.cookie('mini_sidebar') === 'null'){
			$.cookie('mini_sidebar', '1');
		}
	...
	/*-----------------------------------------------------------------------------------*/
	/* 提交执行时，显示loading效果， 限制重复提交
	/*-----------------------------------------------------------------------------------*/
	var actionInLoading = function () {
		jQuery('body .in-loading').click(function () {
            var el = jQuery(this).parents("body");
            App.blockUI(el);
        });
	}
	
	var startBodyLoading = function () {App.blockUI($("body"));}
	var endBodyLoading = function () {App.unblockUI($("body"));}
	...
	var handleBackstretch = function () {
		 $("body").backstretch([
		contextPath+"/img/login/2.jpg", 
		contextPath+"/img/login/3.jpg", 
		contextPath+"/img/login/4.jpg"
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", contextPath+"/css/themes/" + color + ".css");
	...
	/*	将一级菜单的都取消并赋值
	/*-----------------------------------------------------------------------------------*/
	var activeHasSub = function () {
		$("#sidebar ul li").removeClass("active");
		$("#sidebar ul li .arrow").removeClass("open");
		$("#"+hasSub).addClass("active open");
		$("#"+hasSub).find(".arrow").addClass("open");
	}
	
	/*-----------------------------------------------------------------------------------*/
	/*	将二级菜单的都取消并赋值
	/*-----------------------------------------------------------------------------------*/
	var currentSubMenu = function () {
		$("#sidebar ul li ul li").removeClass("current");
		if(subMenu!=null){
	...
	return {
        ...
        init: function () {
			...
			if (App.isPage("task_forms")) {
				handleAutosize(); //Function to handle textarea autosize
				handleCountable(); //Function to handle char count
			}
			if (App.isPage("project_forms")) {
				handleAutosize(); //Function to handle textarea autosize
				handleCountable(); //Function to handle char count
			}
			...
			actionInLoading();//提交执行时，显示loading效果， 限制重复提交
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="'+contextPath+'/img/loaders/12.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
